...
class DALLEEncoder(nn.Module):
	...
	device:              torch.device = attr.ib(default=torch.device('cpu'))
	requires_grad:       bool         = attr.ib(default=False)
	use_mixed_precision: bool         = attr.ib(default=False)
...
# 			]))),
			('output', nn.Sequential(OrderedDict([
				('relu', nn.ReLU()),
				('conv', make_conv(2 * self.n_hid, self.vocab_size, 1, use_float16=False)),
			]))),
		]))
...
class Encoder(nn.Module):
    def __init__(self, n_blk_per_group=4, n_hid=256, z_dim=512, **kwargs):
        super().__init__()
        self.encoder = DALLEEncoder(n_blk_per_group=n_blk_per_group,
                                    n_hid=n_hid,
                                    vocab_size=z_dim)
