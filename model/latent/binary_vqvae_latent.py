...
class Latent(nn.Module):
    def __init__(self, n_latents, z_dim, latent_linear=0, **kwargs):
        super().__init__()
        self.bottles = nn.ModuleList([Bottle(2, z_dim) for _ in range(n_latents)])
        
        if latent_linear==0:
            self.linear = nn.Sequential(nn.Linear(z_dim, n_latents*z_dim),
                                        nn.Tanh(),
                                        nn.Linear(n_latents*z_dim, n_latents*z_dim),
                                        nn.Tanh(),
                                        nn.Linear(n_latents*z_dim, n_latents*z_dim),
                                        nn.Tanh(),
                                        nn.Linear(n_latents*z_dim, n_latents*z_dim))
    ...
    def forward(self, data, **kwargs):
        ...
        ''' Get Zi'''
        # (L, NHW,)
        zi = torch.stack(zi, dim=0)
        dec = 2 ** torch.arange(0, len(self.bottles)).to(zi.device)
        # (NHW,)
        data['zi'] = (zi * dec[:, None]).sum(dim=0)
