...
class DecBlock(nn.Module):
    def __init__(self, H, res, from_enc, mixin, n_blocks):
        super().__init__()
        self.base = res
    ...
    def sample(self, x, acts):
        ...
        projections = rand_projections(c*h*w, device=e.device)
        z_proj = z_reshaped @ projections.T
        e_proj = e @ projections.T
        
        distance = torch.sort(z_proj.T, dim=1)[0] - torch.sort(e_proj.T, dim=1)[0]
        distance = torch.abs(distance) ** 2
        swae = torch.mean(distance)
    ...
    def forward(self, xs, activations, drop_layer, get_latents=False):
        ...
        if self.from_enc:
            ...
            z_copy = z
            z = z * drop_layer[:, None, None, None]
            x = x + self.z_fn(z)
        x = self.resnet(x)
        xs[self.base] = x
        if get_latents:
            return xs, dict(z=z_copy, swae=swae)
        return xs, dict(swae=swae)
...
class VDDecoder(HModule):
    ...
    def build(self):
        ...
        for idx, (res, mixin) in enumerate(blocks):
            dec_blocks.append(DecBlock(H, res, res in H.activation_resolutions, mixin, n_blocks=len(blocks)))
            resos.add(res)
