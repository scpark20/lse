...
import numpy as np
from util.mmd_penalty_multi import mmd_penalty_multi
...
class DecBlock(nn.Module):
    ...
    def sample(self, x, acts):
        ...
        e = torch.randn_like(z_reshaped)*2-1
        # (b,)
        mmd = mmd_penalty_multi(z_reshaped, e, self.H.opts)
        return z, x, mmd
    ...
    def forward(self, xs, activations, get_latents=False):
        ...
        z, x, mmd = self.sample(x, acts)
        x = x + self.z_fn(z)
        x = self.resnet(x)
        xs[self.base] = x
        if get_latents:
            return xs, dict(z=z.detach(), mmd=mmd)
        return xs, dict(mmd=mmd)
