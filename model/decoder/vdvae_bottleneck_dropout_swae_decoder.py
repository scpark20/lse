...
class DecBlock(nn.Module):
    ...
    def forward(self, xs, activations, drop_layer, get_latents=False):
        ...
        #probabilities = torch.rand(len(z))
        # (b, 1, h, w)
        #zeros = sample_binary_outcomes_torch(probabilities, h*w).reshape(b, 1, h, w)
        
        drop_range = torch.randint(0, h*w, size=(b,))
        zeros = (torch.linspace(0, h*w-1, h*w).unsqueeze(0).repeat(b, 1) <  drop_range[:, None]).reshape(b, 1, h, w)
        z_copy = z
        z = z * zeros.to(z.device) * drop_layer[:, None, None, None]
        x = x + self.z_fn(z)
        x = self.resnet(x)
        xs[self.base] = x
        if get_latents:
            return xs, dict(z=z_copy.detach(), swae=swae)
