...
class DecBlock(nn.Module):
    ...
    def get_kl(self, z):
        ...
        z = z.unsqueeze(1)
        # (1, m, z_dim, h, w)
        e = (torch.rand(1, self.M, z_dim, h, w)*2-1).to(z.device)
        # (n, m, z_dim, h, w)
        distance = (z - e) ** 2
        # (m, z_dim, h, w)
    ...
    def sample_uncond(self, x, t=None, lvs=None):
        ...
        if lvs is not None:
            ...
            # (n, z, h, w)
            z = (torch.rand_like(pm)*2-1).to(pm.device)
    ...
    def forward(self, xs, activations, layer_dropout, get_latents=False):
        ...
        # (n, c, h, w)
        dropout = torch.rand_like(z) < probs[:, None, None, None]
        z = z * dropout * layer_dropout[:, None, None, None]
...
class VDDecoder(HModule):
    ...
    def forward(self, activations, get_latents=False, **kwargs):
        ...
        tensor = activations[list(activations.keys())[0]] 
        N = len(tensor)
        # (N,)
        index = torch.randint(0, len(self.dec_blocks), size=(N,))
        # (N, n_layers)
        dropout_layer = torch.linspace(0, len(self.dec_blocks)-1, len(self.dec_blocks)).unsqueeze(0).repeat(N, 1)
        for i, block in enumerate(self.dec_blocks):
            xs, block_stats = block(xs, activations, (dropout_layer[:, i] < index).to(tensor.device), get_latents=get_latents)
