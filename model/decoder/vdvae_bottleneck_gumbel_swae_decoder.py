...
class DecBlock(nn.Module):
    ...
    def sample(self, x, acts):
        ...
        feats = self.prior(x)
        logit, _, xpp = feats[:, :self.zdim, ...], feats[:, self.zdim:self.zdim * 2, ...], feats[:, self.zdim * 2:, ...]
        x = x + xpp
        b, c, h, w = z.size()
    ...
    def forward(self, xs, activations, drop_layer, get_latents=False):
        ...
        z, x, swae, logit = self.sample(x, acts)
        
        z_copy = z
        u = torch.rand_like(z)
        eps = 1e-20
        g = -torch.log(-torch.log(u+eps)+eps)
        mask = torch.sigmoid(logit + g)
        mask_loss = torch.mean(mask)
        mask = mask + ((mask > 0.5).float() - mask).detach()
        z = z * mask * drop_layer[:, None, None, None]
        x = x + self.z_fn(z)
        x = self.resnet(x)
        xs[self.base] = x
        if get_latents:
            return xs, dict(z=z_copy.detach(), logit=logit.detach(), swae=swae, mask_loss=mask_loss)
        return xs, dict(swae=swae, logit=logit.detach(), mask_loss=mask_loss)
    ...
    def forward_uncond(self, xs, t=None, lvs=None):
        ...
        if self.mixin is not None:
            x = x + F.interpolate(xs[self.mixin][:, :x.shape[1], ...], scale_factor=self.base // self.mixin)
        z, x, logit = self.sample_uncond(x, t, lvs=lvs)
        u = torch.rand_like(z)
        eps = 1e-20
        g = -torch.log(-torch.log(u+eps)+eps)
        mask = torch.sigmoid(logit + g)
