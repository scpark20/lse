...
class Decoder(nn.Module):
    def __init__(self, n_blk_per_group=2, n_hid=256, z_dim=512, **kwargs):
        super().__init__()
        self.decoder = DALLEDecoder(n_blk_per_group=n_blk_per_group,
                                    n_hid=n_hid,
                                    vocab_size=z_dim)
        
    def forward(self, data, **kwargs):
        # x : (b, c, h, w)
        data['y'] = F.sigmoid(self.decoder(data['z']))
