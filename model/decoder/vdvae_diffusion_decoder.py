...
class DecBlock(nn.Module):
    def __init__(self, H, res, mixin, n_blocks, draw_samples):
        super().__init__()
        self.base = res
        self.mixin = mixin
        self.H = H
...
class VDDecoder(HModule):
    ...
    def build(self):
        ...
        for idx, (res, mixin) in enumerate(blocks):
            draw_samples = H.draw_samples_list[idx]
            dec_blocks.append(DecBlock(H, res, mixin, n_blocks=len(blocks), draw_samples=draw_samples))
            resos.add(res)
    ...
    def forward_uncond(self, n, t=None, y=None):
        ...
        for idx, block in tqdm(enumerate(self.dec_blocks)):
            try:
                temp = t[idx]
            except TypeError:
                temp = t
